...
def pull_data():
    loctype = raw_input('What type of location descriptor are you using? Please enter a number:\n(1) City Name\n(2) Zip Code\n(3) City ID\n(4) Latitude/Longitude\n')
    if loctype == '1':
        n = "q=" + raw_input('What is the name of the city? If the city name is more than one word, please use hyphens instead of spaces, and leave out periods. (ie. \"St. Petersburg\" would become \"St-Petersburg\")\n')
    ...
    url = 'http://api.openweathermap.org/data/2.5/forecast?' + n + '&units=imperial'
    try:
        res = urlopen(url)
        fct_raw = res.read()
        fct = eval(fct_raw)
